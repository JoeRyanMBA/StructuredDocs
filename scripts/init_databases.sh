...
echo ""
echo "📋 Next steps:"
echo "   1. Copy the matching envs/*.env.example to each droplet as /opt/structureddocs/backend.env"
echo "   2. Set DATABASE_URL in each env file using the user/password above"
echo "   3. Run ./scripts/deploy.sh <environment> <droplet-ip> for each droplet"
